      ...
      <div style={{ opacity: isLoading ? 0 : 1, transition: "opacity 0.5s ease-out" }}>
        <div className="min-h-screen bg-[#0a0a0a] text-gray-300 font-sans relative overflow-hidden selection:bg-[#b4e650]/30 flex flex-col lg:block">
          ...
          <motion.div 
            ...
            className="relative w-full h-[55vh] lg:absolute lg:inset-0 lg:h-auto z-0 pointer-events-none overflow-hidden flex-shrink-0"
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black lg:bg-gradient-to-r lg:from-black/30 lg:via-transparent lg:to-transparent"></div>
          </motion.div>
          ...
          <div className="relative z-10 flex flex-col lg:block flex-grow h-full min-h-screen">
            <AnimatePresence mode="wait">
              {currentPage === 0 ? (
                <AboutPage key="about" />
              ) : (
                <ProjectsPage key="projects" onNavigate={() => setCurrentPage(0)} />
              )}
            </AnimatePresence>
          </div>
        </div>
